...
def test_greedy_scheduler_runtime():
    ...
    assert execution_time < 0.1, f"Greedy Scheduler is too slow: {execution_time:.4f} seconds"
...
def test_local_search_runtime():
    ...
    assert execution_time < 2.0, f"Local Search is too slow: {execution_time:.4f} seconds"